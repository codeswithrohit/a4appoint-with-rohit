...
const Navbar = ({ logout, user, cart, addToCart, removeFromCart, clearCart, subTotal }) => {
  ...
  return (
    ...
    {!menubar && <span onMouseOver={() => { setDropdown(true) }} onMouseLeave={() => { setDropdown(false) }} className='fixed right-9 top-4 z-30 cursor-pointer'>
              {dropdown && <div  className='absolute right-5 bg-white shadow-lg border top-5 rounded-md px-5 py-4 w-32 z-30'>
                <ul>
                  <Link href={'/myaccount'}><a><li className='py-1 hover:text-green-600 text-sm font-bold'>My Account</li></a></Link>
                  <Link href={'/orders'}><a><li className='py-1 hover:text-green-600 text-sm font-bold'>Appointments</li></a></Link>
                  <a onClick={logout} ><li className='py-1 hover:text-green-600 text-sm font-bold cursor-pointer'>Logout</li></a>
                </ul>

              </div>}
              {user.value && <RiAccountCircleFill className="text-xl md:text-2xl mx-6 " />}
    ...
    <div className={`sticky top-0 bg-white z-10 ${!menubar && 'overflow-hidden'}`}>
      <header className="text-gray-600 body-font">
        ...
        <ImMenu onClick={menuCart} className="text-2xl cursor-pointer md:text-4xl mx-8 text-green-500" />
    ...
    <nav className="md:ml-auto md:mr-auto flex flex-wrap items-center text-base justify-center">
      <a href={'/contactus'} className="mr-5 text-blue-400 hover:text-green-600 cursor-pointer">
          <h1 className='font-bold text-xl'>Contact Us</h1>
        </a>
      <a href={'/faq'} className="mr-5 text-green-400 hover:text-blue-500 cursor-pointer">
