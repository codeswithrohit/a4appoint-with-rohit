...
const MyOrder = ({order}) => {
  const hospitals = order.hospitals;
  ...
  return (
    <div className='min-h-screen '>
      ...
      <div className="px-4 py-5 sm:px-6">
        <h3 className="text-lg font-medium leading-6 text-gray-900">{hospitals[key].name} </h3>
        <p className="mt-1 max-w-2xl text-sm text-gray-500">({hospitals[key].date}-{hospitals[key].variant})</p>
      </div></div>})}
      <div className="border-t border-gray-200">
        <dl>
          ...
          <p className="leading-relaxed text-red-400 mb-4">⋆ Your Appointment has been successfully Booked.</p>
